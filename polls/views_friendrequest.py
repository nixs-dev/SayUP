...
def get_friendrequests(request):
    ...
    requests_sent = FriendRequest.objects.filter(sender_id=request.session['user']['id']).values()
    for r in requests_sent:
        r['recipient_username'] = User.objects.get(id=r['recipient_id']).username 
    
    requests_received = FriendRequest.objects.filter(recipient_id=request.session['user']['id']).values()
